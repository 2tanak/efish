                ...
                <div class="table__block--content">
                  ...
                  >
                    <template v-slot:item.name="{ item }">
                      <router-link
                        :href="'/' + $i18n.locale + '/account/reporting/' + item.id"
                    ...
                    <template v-slot:item.action="{ item }">
                      <router-link
                        :to="
                          '/' + $i18n.locale + '/account/reporting/' + 3 + '-rh' + `/${item.id}`
                        "
                      >
                        <svg
                          width="18"
                      ...
                      <a
                        ...
                        :href="'https://api.efish.airweb.kz/api/v1/subject/pdf/3?id=' + item.id"
                        target="_blank"
                        download
...
export default {
  ...
  methods: {
    async apiGetReportingList() {
      try {
        const response = await api.get('subject/list/3');
        if (response.data) {
  ...
  async beforeMount() {
    await this.apiGetReportingList();
  },
};
</script>
