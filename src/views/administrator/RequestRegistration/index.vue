              ...
              <div class="card__content-header">
                <div class="content__title">
                  <div class="content__title--element">
                    ...
                    <div class="content__title--subtext">Субъекта рыбного хозяйства</div>
                  </div>
                </div>
              </div>
                  ...
                  >
                    ...
                    <template v-slot:item.status="{ item }">
                      <a
                        ...
                        :class="{
                          ...
                          'tb__status--error': item.status == 'denied',
                        }"
                      >
                        {{ item.status == 'active' ? 'Активный' : '' }}
                        {{ item.status == 'denied' ? 'Заблокирован' : '' }}
                    ...
                    <template v-slot:item.action="{ item }">
                      <router-link
                        ...
                        :to="'/' + $i18n.locale + '/account/request_registration/' + item.id"
                      ...
                      <!--
                      <a class="tb__table--btn tb__table--edit" @click="$modal.show('modal_success')" v-tooltip.top-center="{content:'Одобрить', class: ['tooltip__btn']}">
                        <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                          <path d="M9 16.5C13.1422 16.5 16.5 13.1422 16.5 9C16.5 4.85775 13.1422 1.5 9 1.5C4.85775 1.5 1.5 4.85775 1.5 9C1.5 13.1422 4.85775 16.5 9 16.5Z" stroke="#52A5FC" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                          <path d="M6.75 9L8.25 10.5L11.25 7.5" stroke="#52A5FC" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                        </svg>
                      </a>
                      -->
                      <a
                        v-if="
                          $store.getters.userPermission(['change_user']) && item.status == 'new'
                        "
...
export default {
  data() {
    return {
      ...
      headers: [
        { text: 'Id', value: 'id' },
        { text: this.$t('headers.fio'), value: 'name', width: '30%' },
        { text: 'ИИН/БИН', value: 'iin_bin' },
        { text: 'Дата подачи заявки', value: 'created_at' },
  ...
  methods: {
    blockElement(evt) {
      ...
      api
        .post('user/register/' + this.blockElemet.id + '/deny', formData, {
          headers: {
    ...
    apiGetUserList(page = 1, per_page = 10) {
      api
        .get('user/register/list', { params: { page, per_page } })
        .then((response) => {
          if (response.data) {
  ...
  head: {
    title() {
      return {
        inner: this.$t('sidebar.request_registration'),
      };
    },
    meta: [],
  },
