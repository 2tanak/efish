...
export default {
  ...
  computed: {
    ...mapGetters(['_getDistricts']),
    formList() {
      if (this.categoryValue) {
        let elements = [];
        elements = this._getDistricts.filter((element) => {
          if (element.region) {
            if (!Array.isArray(this.categoryValue) && element.region.id == this.categoryValue) {
              return true;
            } else if (Array.isArray(this.categoryValue)) {
              return this.categoryValue.find((category) => {
                if (category == element.region.id) {
                  return true;
                }
              });
            }
      ...
      //   let list = this._getDistricts.filter((element) => {
  ...
  methods: {
    ...mapActions(['_fetchDistricts']),
    setValues() {
      if (this._getDistricts) {
        let multi = [];
        this._getDistricts.forEach((e) => {
